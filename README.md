...
- Приоритет №4: параметризация из общего файла `config.json` (коммитится в GitLab)
- Приоритет №3: параметризация из локального файла `config.local.json` (не коммитится в GitLab)
...
```
 $ python -m pytest --username=<value> --<password>=<valuе>
```
